...
def alphabeta_min_node(board, color, alpha, beta, limit, caching=0, ordering=0):
    ...
    if ordering:
        moves.sort(key=lambda m: compute_heuristic(play_move(board, opponent, m[0], m[1]), opponent), reverse=True)
...
def alphabeta_max_node(board, color, alpha, beta, limit, caching=0, ordering=0):
    ...
    if ordering:
        moves.sort(key=lambda m: compute_heuristic(play_move(board, color, m[0], m[1]), color), reverse=True)
